...
export default function PrototypePF() {
  ...
  const [currentUan, setCurrentUan] = usePersistentState(
    "epfo_uan",
    DEFAULT_USER_UAN,
  );
  const activeUser =
    SYNTHETIC_USERS[currentUan] || SYNTHETIC_USERS[DEFAULT_USER_UAN];
  ...
  const handleLogin = useCallback(
    (uan) => {
      ...
      const matchedUan =
        Object.keys(SYNTHETIC_USERS).find((u) => u === cleanUan) ||
        DEFAULT_USER_UAN;
  ...
  return (
    <div
      className={`min-h-screen flex flex-col bg-[#f5f5f5] font-sans text-[15px] selection:bg-[#048282] selection:text-white`}
    >
