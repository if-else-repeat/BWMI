...
import { saveDraft, getDraft, clearDraft, enqueueOfflinePayload, logAuditEvent, SYNTHETIC_USERS, DEFAULT_USER_UAN } from '../db/dexie';
import { 
  Building2, 
  Car, 
  FileSpreadsheet, 
...
const DEPARTMENTS = [
  {
    ...
    code: 'CBDT-GRV-01',
    fields: [
      { name: 'pan', label: 'Permanent Account Number (PAN)', placeholder: 'ABCDE1234F', type: 'text' },
      { name: 'ay', label: 'Assessment Year', placeholder: '2025-26', type: 'text' },
      { name: 'grievanceCategory', label: 'Grievance Category', type: 'select', options: ['Delayed Refund (> 90 Days)', 'e-Verification Mismatch', 'Rectification Request (u/s 154)', 'Challan / Tax Credit Mismatch'] },
      { name: 'claimAmount', label: 'Disputed Amount (₹)', placeholder: '24500', type: 'number' }
    ]
  },
  {
    id: 'PARIVAHAN',
    ...
    code: 'MORTH-DL-04',
    fields: [
      { name: 'dlNumber', label: 'Driving License Number', placeholder: 'KA-01-2018-0094821', type: 'text' },
      { name: 'rtoJurisdiction', label: 'RTO Jurisdiction Code', placeholder: 'KA-01 (Koramangala, Bengaluru)', type: 'text' },
      { name: 'newAddress', label: 'New Permanent Residential Address', placeholder: '#42, 80 Feet Road, 4th Block, Koramangala', type: 'text' },
      { name: 'pincode', label: 'Postal PIN Code', placeholder: '560034', type: 'number' }
    ]
  },
  {
    id: 'NFSA',
    ...
    code: 'NFSA-PDS-08',
    fields: [
      { name: 'rationCardNo', label: 'Digital Ration Card Number', placeholder: 'RC-9928-1002-4910', type: 'text' },
      { name: 'headAadhaar', label: 'Head of Family Aadhaar', placeholder: 'XXXX-XXXX-8921', type: 'text' },
      { name: 'newMemberName', label: 'New Beneficiary Full Name', placeholder: 'Aarav Kumar', type: 'text' },
      { name: 'relationship', label: 'Relationship to Family Head', type: 'select', options: ['Son / Daughter', 'Spouse', 'Dependent Parent', 'Other'] }
    ]
  }
];
...
export default function PrototypeGateway() {
  ...
  // App State
  const [activeDeptId, setActiveDeptId] = usePersistentState('gw_dept', DEPARTMENTS[0].id);
  const [formData, setFormData] = useState({});
  const [lastSaved, setLastSaved] = useState(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionSuccess, setSubmissionSuccess] = useState(null);
  const [offlineAlert, setOfflineAlert] = useState(null);
  const [isAuthenticated, setIsAuthenticated] = usePersistentState('gw_auth', false);
    ...
    async function loadServiceDraft() {
      ...
      if (draft && draft.formData) {
        ...
        activeDept.fields.forEach((f) => {
          ...
          else if (f.name === 'headAadhaar') initial[f.name] = activeUser.aadhaarMasked;
          else initial[f.name] = '';
        });
        setFormData(initial);
        setLastSaved(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isOnline) {
        ...
        await logAuditEvent('GATEWAY_SUBMIT_ONLINE', { dept: activeDept.id, ackNumber }, 'ONLINE');
        
        setSubmissionSuccess(ackNumber);
        addToast({ type: 'success', title: 'SUBMISSION SUCCESSFUL', message: `Acknowledgement ID: ${ackNumber}` });
        confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } });
      } else {
        const queueResult = await enqueueOfflinePayload(draftKey, formData, activeUser.uan);
        await clearDraft(draftKey, activeUser.uan);
        
        setOfflineAlert(queueResult.referenceId);
        addToast({ type: 'warning', title: 'NETWORK UNREACHABLE', message: 'Payload queued to IndexedDB for background sync.' });
      }
    } catch (err) {
      console.error(err);
    } finally {
    ...
    return (
      <div className="min-h-screen bg-[#f4f4f0] flex flex-col items-center justify-center p-4 font-sans selection:bg-black selection:text-white">
        <div className="max-w-md w-full bg-white brutal-border brutal-shadow p-8 text-center space-y-6">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase">Unified Citizen Gateway</h1>
            <p className="text-sm text-gray-600 mt-1 font-medium">One Nation, One Portal Architecture</p>
          </div>
          
          <div className="p-4 bg-gray-50 border-2 border-dashed border-gray-300">
            <p className="text-xs text-gray-500 mb-3 font-mono">TEST ENVIRONMENT (LOCAL STATE)</p>
            <button 
      ...
      <header className="bg-black text-white px-4 py-3 brutal-border-b sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-black uppercase tracking-wider text-sm md:text-base">Unified Citizen Gateway</h1>
              <div className="text-[10px] text-gray-400 font-mono tracking-widest uppercase">Govt. of India Scalability Prototype</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-2 text-xs font-mono">
              ...
              <span className="text-gray-300">Autosave: {lastSaved || 'Ready'}</span>
            </div>
            ...
            >
              ...
              <span className="hidden sm:inline">{isOnline ? 'Online Mode' : 'Offline Mode'}</span>
            </button>

            <button onClick={() => setIsAuthenticated(false)} className="text-gray-400 hover:text-white transition-colors">
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full flex flex-col md:flex-row">
        ...
        <nav className="w-full md:w-64 bg-white brutal-border-r p-4 md:min-h-[calc(100vh-65px)] flex flex-col gap-2">
          <div className="p-4 bg-gray-50 brutal-border mb-4">
            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Active Citizen</div>
            <div className="font-black text-sm uppercase">{activeUser.name}</div>
            <div className="font-mono text-xs text-gray-600 mt-0.5">ID: {activeUser.aadhaarMasked}</div>
          </div>

          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider px-2 mb-1 mt-2">Department Services</div>
          ...
          {DEPARTMENTS.map(dept => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-white' : ''}`} style={{ color: isActive ? '#fff' : dept.color }} />
                <span>{dept.name}</span>
              </button>
            );
          })}
        ...
        <main className="flex-1 p-4 md:p-8">
          ...
          <div className="max-w-3xl">
            <div className="flex items-center justify-between mb-6 border-b-2 border-black pb-2">
              <h3 className="text-xl font-black uppercase">{activeDept.service}</h3>
              <span className="bg-black text-white text-[10px] px-2 py-0.5 font-bold uppercase">Form Entry</span>
            </div>
            ...
            {submissionSuccess ? (
              <div className="bg-white p-8 brutal-border brutal-shadow border-t-8 border-t-[#10b981] animate-in fade-in zoom-in-95">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-2xl font-black uppercase">Service Request Filed</h3>
                    <p className="text-gray-600 font-medium">{activeDept.service}</p>
                  </div>
                </div>
                <div className="bg-gray-50 p-4 brutal-border">
                  <div className="text-xs text-gray-500 font-bold uppercase mb-1">Acknowledgement Number</div>
                  <div className="text-xl font-mono font-black">{submissionSuccess}</div>
                </div>
                <button 
              ...
              <div className="bg-white p-8 brutal-border brutal-shadow border-t-8 border-t-[#f59e0b] animate-in fade-in zoom-in-95">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="text-2xl font-black uppercase">Offline Queue Active</h3>
                    <p className="text-gray-600 font-medium">Server Unreachable (Simulated 504)</p>
                  </div>
                </div>
                <p className="mb-6 font-medium text-gray-700 leading-relaxed">
                  Your form data has been securely saved to the local IndexedDB. It will automatically sync to the {activeDept.name} servers when connection is restored.
                </p>
                <div className="bg-amber-50 p-4 brutal-border border-amber-200">
                  <div className="text-xs text-amber-700 font-bold uppercase mb-1">Local Reference ID</div>
                  <div className="text-lg font-mono font-black text-amber-900">{offlineAlert}</div>
                </div>
                <button 
              ...
              <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 brutal-border brutal-shadow space-y-6 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="bg-gray-50 p-4 border border-gray-200 text-xs text-gray-600 font-medium">
                  By submitting this form, I solemnly declare that the information provided is true to the best of my knowledge, and I understand this data is persisted securely on my device before transmission.
                </div>
                ...
                <div className="pt-4 border-t-2 border-black flex items-center justify-between">
                  ...
                  >
                    {isSubmitting ? 'Processing...' : (
                      <>
                        {isOnline ? 'Submit to Govt Gateway' : 'Queue to Local IndexedDB'}
                        {isOnline ? <ArrowRight className="w-4 h-4" /> : <Database className="w-4 h-4" />}
                      </>
                    )}
                  </button>
                </div>
