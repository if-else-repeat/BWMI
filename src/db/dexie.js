...
export class BharatResilienceDB extends Dexie {
  constructor() {
    ...
    this.version(2).stores({
      ...
      syncQueue:
        "++id, serviceType, referenceId, queuedAt, status, attempts, userUan",
      submissions:
        "++id, referenceId, serviceType, submittedAt, ackNumber, isSynced, userUan",
...
export const SYNTHETIC_USERS = {
  101294829101: {
    ...
    totalPfBalance: 148200.0,
    employeeShare: 102400.0,
    employerShare: 33800.0,
    pensionShare: 12000.0,
    ...
    passbookEntries: [
      ...
      {
        month: "INTEREST CREDIT (FY 25-26 @ 8.25%)",
        wage: "-",
        empShare: "+ ₹7,840",
        emplyrShare: "+ ₹3,400",
        pension: "-",
  ...
  101940281192: {
    ...
    totalPfBalance: 18500.0,
    employeeShare: 12500.0,
    employerShare: 4200.0,
    pensionShare: 1800.0,
...
export async function enqueueOfflinePayload(
  serviceType,
  payload,
  userUan = DEFAULT_USER_UAN,
) {
  try {
    ...
    await logAuditEvent(
      "CLAIM_QUEUED_OFFLINE",
      { referenceId, serviceType, id, userUan },
      "OFFLINE",
    );
  ...
  try {
    ...
    for (const item of queuedItems) {
      ...
      await logAuditEvent(
        ...
        { referenceId: item.referenceId, ackNumber },
        "ONLINE_RECONNECTED",
      );
