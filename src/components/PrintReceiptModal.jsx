...
export default function PrintReceiptModal({ submission, onClose }) {
  ...
  const handleDownloadJson = () => {
    ...
    downloadAnchor.setAttribute(
      "download",
      `EPFO_Claim_Receipt_${submission.ackNumber}.json`,
    );
        ...
        <div className="p-6 overflow-y-auto space-y-6 bg-white font-mono text-xs">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-y-2 text-[11px]">
              <div>
                <span className="text-gray-500">Universal A/c No (UAN):</span>
              </div>
              <div className="font-bold text-right text-black">
                {submission.payload?.uan || "1000 2349 001"}
              </div>
              ...
              <div className="font-bold text-right text-black">
                {submission.payload?.name || "Ramesh Kumar"}
              </div>

              <div>
                <span className="text-gray-500">Advance Reason / Section:</span>
              </div>
              <div className="font-bold text-right text-black">
                {submission.payload?.reason || "Para 68J (Medical)"}
              </div>

              <div>
                <span className="text-gray-500">Claim Amount:</span>
              </div>
              <div className="font-bold text-right text-base text-black font-mono">
                ₹{submission.payload?.amount}
              </div>
              ...
              <div className="text-right text-black">
                {submission.payload?.bankAccountNumber || "•••••••••8237"}
              </div>
          ...
          <div className="p-3 bg-black text-[#f4f4f0] space-y-2 brutal-border">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] bg-[#10b981] text-black px-1 font-bold">
                VERIFIED
              </span>
            </div>
            <div className="font-mono text-[10px] text-green-400 break-all">
              {submission.receiptHash ||
                "SHA256-e9b41a8c903ef821d74a6b5c01f92e34"}
            </div>
            <div className="text-[9px] text-gray-400">
              * This submission was preserved in client IndexedDB and replayed
              through the resilient sync worker without data loss.
            </div>
          </div>
        </div>
