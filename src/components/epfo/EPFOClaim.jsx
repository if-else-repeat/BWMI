...
const ADVANCE_REASONS = [
  {
    id: "68J_MEDICAL",
    label: "Illness / Medical Treatment (Para 68J)",
    maxLimit: 75000,
    eligibility: "No minimum service • Self-declaration",
    fastTrack: true,
  },
  {
    id: "68B_HOUSING",
    label: "Purchase/Construction of House (Para 68B)",
    maxLimit: 125000,
    eligibility: "5+ years service • Up to 90% balance",
    fastTrack: false,
  },
  {
    id: "68K_EDUCATION",
    label: "Higher Education of Children (Para 68K)",
    maxLimit: 49200,
    eligibility: "7+ years service • Up to 50% employee share",
    fastTrack: false,
  },
  {
    id: "SPECIAL_RELIEF",
    label: "Special Calamity / Non-Refundable Advance",
    maxLimit: 73800,
    eligibility: "Zero documentation • 48-hr credit",
    fastTrack: true,
  },
];
...
export default function EPFOClaim({ activeUser, lang, currentUan }) {
  ...
  const handleSubmit = async () => {
    ...
    const selectedReason = ADVANCE_REASONS.find(
      (r) => r.id === formData.reason,
    );
    if (selectedReason && amountVal > selectedReason.maxLimit) {
      setValidationError(
        `Amount exceeds the maximum limit for this reason (₹${selectedReason.maxLimit}).`,
      );
      return;
    }
    ...
    try {
      if (isOnline) {
        ...
        const receiptHash = `SHA256-${Array.from(
        ...
        await logAuditEvent(
          "CLAIM_SUBMITTED_ONLINE",
          { ackNumber, amount: formData.amount, userUan: currentUan },
          "ONLINE",
        );
        ...
        addToast({
          ...
          message: `Acknowledgement: ${ackNumber}`,
        });
        ...
        addToast({
          ...
          message: "Claim queued to IndexedDB. Will auto-sync when online.",
        });
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="brutal-card bg-white p-6 md:p-8 brutal-border-l-4 border-l-green-600">
        ...
        <div className="bg-green-50 p-4 border border-green-200 brutal-border mb-6">
          <p className="text-sm text-gray-600 mb-1">
            Acknowledgement Tracking ID:
          </p>
          <p className="text-xl font-mono font-bold text-gray-900">
            {submissionSuccess.ackNumber}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          <div className="p-4 bg-gray-50 border border-gray-200">
            ...
            <p className="font-bold text-lg">
              XXXXX{String(formData.bankAccountNumber).slice(-4)}
            </p>
          </div>
          <div className="p-4 bg-gray-50 border border-gray-200">
            <p className="text-xs text-gray-500 uppercase font-semibold">
            ...
            <p className="font-bold text-lg flex items-center gap-2">
              <Fingerprint className="h-4 w-4" /> WebAuthn Passkey
            </p>
          </div>
          <div className="p-4 bg-gray-50 border border-gray-200">
            <p className="text-xs text-gray-500 uppercase font-semibold">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            ...
            className="brutal-btn bg-teal-700 text-white flex items-center justify-center gap-2"
          >
            <Printer className="h-5 w-5" /> View & Print Receipt
          </button>
          <button
            onClick={resetForm}
            className="brutal-btn bg-gray-200 text-gray-800 flex items-center justify-center gap-2"
          >
    ...
    return (
      <div className="brutal-card bg-amber-50 p-6 md:p-8 brutal-border-l-4 border-l-amber-600">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-2">
              Network Disconnected: Claim Queued
            </h2>
            <p className="text-gray-700 mb-4">
              Your device is currently offline or the server is unreachable
              (simulating a 504 Gateway Timeout). Your Form 31 claim has been
              securely saved in IndexedDB and will be submitted automatically
        ...
        <div className="mt-6 pt-6 border-t border-amber-200">
          <button
            onClick={resetForm}
            className="text-amber-800 font-semibold hover:underline flex items-center gap-2"
          >
  ...
  const selectedReasonData = ADVANCE_REASONS.find(
    (r) => r.id === formData.reason,
  );
    ...
    <div className="brutal-card bg-white brutal-shadow">
      ...
      <div className="border-b-2 border-gray-900">
        <div className="p-4 md:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50">
          <div>
            <h1 className="text-2xl font-serif font-bold text-[#048282]">
              Online Claim (Form 31)
            </h1>
            <p className="text-sm text-gray-600 mt-1">
              PF Advance / Non-Refundable Withdrawal
            </p>
          </div>
          <div className="mt-4 sm:mt-0 flex items-center text-xs text-gray-500 bg-white px-3 py-1.5 border border-gray-300">
            {lastSavedTime ? (
              <span className="flex items-center gap-1.5">
                <CheckCircle2 className="h-3.5 w-3.5 text-green-600" /> Draft
        ...
        <div className="flex text-sm font-semibold divide-x-2 divide-gray-900 border-t-2 border-gray-900">
          <div
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 ${currentStep === 1 ? "bg-gray-900 text-white" : currentStep > 1 ? "bg-green-50 text-green-700" : "bg-white text-gray-500"}`}
          >
            <span className="hidden sm:inline">STEP</span> 01{" "}
            {currentStep > 1 && <CheckCircle2 className="h-4 w-4 ml-1" />}
          </div>
          <div
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 ${currentStep === 2 ? "bg-gray-900 text-white" : currentStep > 2 ? "bg-green-50 text-green-700" : "bg-white text-gray-500"}`}
          >
            <span className="hidden sm:inline">STEP</span> 02{" "}
            {currentStep > 2 && <CheckCircle2 className="h-4 w-4 ml-1" />}
          </div>
          <div
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 ${currentStep === 3 ? "bg-gray-900 text-white" : "bg-white text-gray-500"}`}
          >
      ...
      <div className="p-4 md:p-6 lg:p-8">
        ...
        {currentStep === 1 && (
          <div className="space-y-6">
            <h2 className="text-lg font-bold border-b border-gray-200 pb-2 mb-4 text-[#C1622D]">
              Member Details
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              <div>
                ...
                <div className="font-mono bg-gray-50 p-2 border border-gray-200">
                  {formData.uan}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="bg-gray-50 p-2 border border-gray-200">
                  {activeUser?.fatherName || "Not Available"}
                </div>
              </div>
              <div>
                <label className="block text-xs text-gray-500 font-semibold mb-1">
                ...
                <div className="bg-gray-50 p-2 border border-gray-200">
                  {activeUser?.dob || "01-01-1990"} •{" "}
                  {activeUser?.gender || "M"}
                </div>
              </div>
            </div>

            <h2 className="text-lg font-bold border-b border-gray-200 pb-2 mt-8 mb-4 text-[#C1622D]">
            ...
            <div className="flex justify-end mt-8 pt-6 border-t border-gray-200">
              <button
                onClick={handleNext}
                className="brutal-btn bg-teal-700 text-white flex items-center gap-2"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {ADVANCE_REASONS.map((reason) => (
                <label
                  key={reason.id}
                  className={`flex items-start p-4 border-2 cursor-pointer transition-colors ${formData.reason === reason.id ? "border-teal-700 bg-teal-50" : "border-gray-200 hover:border-gray-300"}`}
                >
                  <input
                    type="radio"
                    name="reason"
                  ...
                  <div className="ml-3 flex-1">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-2 text-sm">
                      ...
                      <span className="font-semibold text-teal-800">
                        Max Limit: ₹{reason.maxLimit.toLocaleString("en-IN")}
                      </span>
                    </div>
                  </div>
                </label>
            ...
            {formData.reason && (
              <div className="mt-8 p-6 bg-gray-50 border border-gray-200">
                ...
                <p className="mt-2 text-xs text-gray-600">
                  ...
                  {selectedReasonData?.maxLimit.toLocaleString("en-IN")}) or
            ...
            <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
              <button
                onClick={handleBack}
                className="brutal-btn bg-white border-2 border-gray-900 text-gray-900 flex items-center gap-2"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border-2 border-gray-900 overflow-hidden">
              <table className="w-full text-left border-collapse">
                <tbody className="divide-y divide-gray-200">
                  <tr>
                    <th className="py-3 px-4 bg-gray-50 text-sm font-semibold text-gray-600 w-1/3 border-r border-gray-200">
                      UAN
                    </th>
                  ...
                  <tr>
                    <th className="py-3 px-4 bg-gray-50 text-sm font-semibold text-gray-600 border-r border-gray-200">
                      Advance Reason
                    </th>
                  ...
                  <tr>
                    ...
                    <td className="py-3 px-4 font-mono font-bold text-teal-800 text-lg">
                      ₹
                  ...
                  <tr>
                    ...
                    <td className="py-3 px-4 font-mono text-gray-900">
                      {formData.bankAccountNumber} (IFSC: {formData.ifsc})
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="bg-green-50 p-4 border border-green-200 flex items-center gap-3 mt-4">
              ...
              <div>
                ...
                <p className="text-xs text-green-700">
                  Authenticated via WebAuthn on this device.
                </p>
              </div>
            </div>
            ...
            <label className="flex items-start gap-3 mt-6 p-4 border border-gray-200 bg-gray-50 cursor-pointer">
              ...
              <span className="text-sm text-gray-800 leading-relaxed">
                I hereby declare that the particulars given above are true and
                correct. I consent to seed my Aadhaar for e-KYC authentication.
                I understand that submitting false information may attract penal
                action.
              </span>
            </label>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-8 pt-6 border-t border-gray-200 gap-4">
              <button
                onClick={handleBack}
                className="brutal-btn bg-white border-2 border-gray-900 text-gray-900 flex items-center gap-2 w-full sm:w-auto justify-center"
              >
